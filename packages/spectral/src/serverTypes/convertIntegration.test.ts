...
describe("convertFlow with polling triggers", () => {
  ...
  it("synthesizes the default resolveItems and getNextDiscoveryState on the trigger", () => {
    ...
    // The synthesized getNextDiscoveryState reads the cursor the fire stamped onto the payload,
    // defaulting to null (stop) when absent.
  ...
  it("wraps the fire so items land at body.data and the returned cursor at discoveryState", async () => {
    ...
    // Mid-pagination: items at body.data, next cursor stamped onto discoveryState.
    ...
    // Final page: no returned cursor → discoveryState defaults to null, ending the loop.
