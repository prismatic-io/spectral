...
export interface Input {
  ...
  dataSource?: string;
  shown?: boolean;
  /** Nested child inputs. Populated when `type === "structuredObject"` (the
   * declared children) or `type === "dynamicObject"` (each entry is itself a
   * `structuredObject` representing one configuration option). */
  inputs?: Input[];
}
